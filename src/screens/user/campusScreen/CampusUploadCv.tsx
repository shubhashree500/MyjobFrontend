...
const CampusUploadCv = ({ route, navigation }: any) => {
  ...
  const handleApplyNow = async () => {
    ...
    try {
      ...
      formData.append('compId', events.organization.compId);
      formData.append('jobId', events.jobId);
      ...
      const response = await axios.post(`http://192.168.29.66:6200/api/v1/requestApplication/create`, formData, {
        headers: {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
          <Image
                   source={{
                     uri: events.organization.logo
                       ? `${config.apiUrl}/photo/${events.organization.logo}`
                       : undefined,
                   }}
      ...
      <View style={styles.detailsContainer}>
        <Text style={styles.jobTitle}>{events.degName}</Text>
        <View style={styles.jobDetails}>
          <Text style={styles.detailText}>{events.organization.companyName}</Text>
          <Text style={styles.separator}>•</Text>
          <Text style={styles.detailText}>{events.location}</Text>
          <Text style={styles.separator}>•</Text>
          <Text style={styles.detailText}>{events.postedTime}</Text>
        </View>
      </View>
